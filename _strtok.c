...
/**
 * t_strlen - returns token's str length for mallocing
 * @str: a token
...
int t_size(char *str, char delm)
{
	...
	while (str[i] != '\0')
	{
		...
		{
			/* handles continuous delims */
			num_delm++;
		}
		if ((str[i] == delm) && (str[i + 1] == '\0'))
		{
			/*handles continuous delims after full command */
			num_delm--;
		}
		i++;
	}
 ...
 * @str: user's command typed into shell
 * @delm: delimeter ( " ");
 * Return: an array of tokens like {"ls", "-l", "/tmp"}
 */
char **_str_tok(char *str, char *delm)
{
	...
	/* malloc ptrs to store array of tokens (buffsize + 1) */
	buffsize = t_size(str, d_ch);
	toks = malloc(sizeof(char *) * (buffsize + 2));
	if (toks == NULL)
		return (NULL);
	while (str[se] != '\0')	/* finds string ending index */
		se++;
	while (si < se)
	{ /* malloc lengths for each ptr in array */
		if (str[si] != d_ch)
		{
		...
		/* handles repeated delimeters; increment ptr after ("ls __-l")*/
