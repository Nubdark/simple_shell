...
/**
 * not_found - writes error 
 * @str: user's typed command
 * @c_n: nth user's typed command
 * @env: brings in envtl variables linked list to write shell name
 */
void not_found(char *str, int c_n, list_t *env)
{
	...
	shell = get_env("_", env); /* gets shell name to write */
	while (shell[count] != '\0')
		count++;
	write(STDOUT_FILENO, shell, count);
	free(shell);
	write(STDOUT_FILENO, ": ", 2);
	num = int_to_string(c_n); /* changes cmd line num to string to write */
	count = 0;
	while (num[count] != '\0')
		count++;
