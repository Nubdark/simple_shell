...
char *c_strcat(char *dest, char *src)
{
	...
	/* because dest was malloced outside of function */
 ...
 * @name: envtl variable name (e.g. "OLDPWD")
...
int c_setenv(list_t **env, char *name, char *dir)
{
	...
	cat = _strcat(cat, "=");
	cat = _strcat(cat, dir);
	index = find_env(*env, name); /* get indx to env var in linked list */

	/* traverse to index, free node data, reassign data */
	holder = *env;
	while (j < index)
	{
		holder = holder->next;
 ...
 * @env: bring in envtl linked list to update PATH and OLDPWD
 ...
 * @env: bring in envtl linked list updates PATH and OLDPWD
 * @current: bring in current working dir
 ...
 * @env: envtl linked list to retrieve HOME and OLDPWD paths
 * @num: nth user command used at error message
